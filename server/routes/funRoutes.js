...
const express = require("express");
const multer = require("multer");
const FunPost = require("../models/FunPost");
...
router.get("/", async (req, res) => {
  try {
    const posts = await FunPost.find().sort({ createdAt: -1 });
    res.json(posts);
  } catch (err) {
...
router.post("/", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const newPost = new FunPost({ title, userId, pin, image: imageUrl, likes: [] });
    await newPost.save();
...
router.post("/:id/like", async (req, res) => {
  const { username } = req.body;
  try {
    const post = await FunPost.findById(req.params.id);
    if (!post) return res.status(404).json({ error: "Post not found" });
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    await FunPost.findByIdAndDelete(req.params.id);
